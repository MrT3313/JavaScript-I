...
let lastCar = 0;
console.log(`The 50th (last) car on the lot is a ${inventory[inventory.length - 1].car_make} ${inventory[inventory.length - 1].car_model}`);
...
for (i = 0; i < inventory.length; i++) {
  let currentCar_model = inventory[i].car_model.toLowerCase()

  if (!carModels.includes(currentCar_model)){
    carModels.push(currentCar_model)
  } else {
    console.log(i, currentCar_model, 'is duplicate model')
  }
}
...
console.log(oldCars)
console.log(`There are ${oldCars.length} cars with model years EARLIER than 2000`)
...
console.log(`There are ${BMWAndAudi.length} cars on the lot made by BMW or Audi`)
